...
for (chr in 1:22) {
  ...
  for (i in 1:1000){
    ...
    ###Training set 
    train_ind <- sample(seq_len(nrow(Z_qtl)), size = 500*(1-ratio))
    train_ind_afr <- sample(seq_len(nrow(Z_qtl_afr)), size = 500*ratio)
    
    
    
    Z_qtl_train <-  as.matrix(Z_qtl[train_ind, ])
    Z_qtl_train_afr <-  as.matrix(Z_qtl_afr[train_ind_afr, ])
    
    Z_qtl_train<- rbind(Z_qtl_train ,Z_qtl_train_afr)
    ...
    test_ind <- sample(seq_len(nrow(Z_qtl_test)), size = 250)
    ...
    Z_qtl_test_afr <-  as.matrix(Z_qtl_afr[-train_ind_afr, ])
    test_ind_afr <- sample(seq_len(nrow(Z_qtl_test_afr)), size = 250)
    Z_qtl_afr_test <-  as.matrix(Z_qtl_test_afr[test_ind_afr , ])
  ...
  out.file1 <- paste("/u/scratch/y/yhc1998/proporiton_afr_training/r2_EUR_chr",chr,"_", row, ".txt", sep="")
  out.file2 <- paste("/u/scratch/y/yhc1998/proporiton_afr_training/r2_AFR_chr",chr,"_", row, ".txt", sep="")
